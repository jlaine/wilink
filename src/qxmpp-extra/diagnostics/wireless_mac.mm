...
inline int nsnumberToInt(NSNumber *nsnum)
{
    return nsnum ? [nsnum intValue] : 0;
}

inline QString nsstringToQString(NSString *nsstr)
{
    return nsstr ? cfstringRefToQstring(reinterpret_cast<CFStringRef>(nsstr)) : QString();
}
